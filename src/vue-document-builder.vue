...
<script>
	...
	export default {
		...
		mounted() {
			...
			//Initialize editor
			grapesjs.plugins.add("gjs-document-creator-preset", this.documentCreatorPresetPlugin);
		...
		methods: {
			documentCreatorPresetPlugin(editor, opts = {}) {
				...
				setTimeout(() => {
					...
					el.style.cssText = "font-family: 'Roboto', sans-serif; font-size: 14px; margin: 1.5cm;";
				}, 500);
			},

			searchPlugin(editor) {
				// check if search input is already there
				const searchExist = document.getElementById("custom-search");
				if (searchExist) return;
				const searchInput = document.createElement("input");
				searchInput.setAttribute("id", "custom-search");
			...
			/**
			 * Add the commands to the editor instance
			 * @param {*} editor - GrapesJS instance
			 */
			initCommands(editor) {
